...
Supporta diversi tipi formati e servizi geospaziali, i più noti sono:
...
%Todo: Parlare di workspace, storage etc

GeoServer possiede anche un interfaccia grafica, realizzata come applicazone web, che permete di eseguire tutte le operazioni sopra citate. Supporta

GeoServer utilizza Spring come Framework per gestire la maggior parte delle sue funzionalità, mentre per l'interfaccia utente è stata realizzata tramite l'utilizzo di Wicket. Inoltre GeoServer fornisce anche un'API Rest, implementata usando sempre Spring (Spring MVC Framework) che permette di eseguire tutte le operazioni


L'API REST è implementata utilizzando il framework spring-mvc-framework.
