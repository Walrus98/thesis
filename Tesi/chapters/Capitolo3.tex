...
\\Comportandosi da reverse proxy, reinstrada le richieste ricevute verso il microservizio appropiato, mascherando ai client la pluralità di servizi esistenti. Una volta ricevuta la risposta dal microservizio, la trasmette al client che ne aveva fatto richiesta.
...
\\Tale esempio richiede ovviamente che il nome del layer sia noto: nel caso in cui il client non conosce la lista di layer disponibili, sarà comunque necessario eseguire una richiesta di GetCapabilities.
\\Anche in questo caso, OpenLayer si occupa di eseguire la richiesta e di parsare in automatico il contentuo dell'XML, senza dover creare una struttura dati apposita per manipolare i dati ricevuti da essa.
...
% \begin{figure}[htbp]
%     \centering
%     \includegraphics[width=1\textwidth]{images/Capitolo3/image.png}
%     \caption{Tiles con livello di zoom}
%     \label{fig:tmsTiles}
% \end{figure}


Lo sviluppo dell'integrazione delle mappe è iniziato con OpenLayer,

spiegare perché openLyaer e non un altro vantaggi etc
