...
Il candidato, sotto supervisione del tutor aziendale, ha iniziato creando una nuova pagina web all'interno dell'applicazione, nella quale ha importato il componente della mappa già esistente nell'Ui-Kit, così da poter integrare il supporto ai protocolli OGC richiesti e iniziare a testare le mappe censite.
\\Lo sviluppo è proseguito con estendere il componente Angular delle mappe, che presentava solamente l'integrazione agli elementi base di OpenLayers e un supporto alla sovrapposizione multipla di layer differenti.
\\La prima funzionalità aggiuntata è stata quella del supporto al formato GeoJSON, selezionando come prima mappa da testare quella delle frane in Toscana, fornita da IdroGEO. Questa mappa è stata preferita rispetto alle altre in quanto può essere scaricata localmente e caricata all'interno del progetto, lavorando così in un ambiente locale, senza dover contattare alcun servizio esterno. Ciò ha facilitato il lavoro di programmazione, riducendo il rischio di errore dovuto a eventuali problemi di disservizio dei server di mappe esterni.
\\In questa fase, le parti di codice che hanno subito più modifiche sono state le classi BuildLayer.ts e MapModel.ts, le quali si occupano di far istanziare l'oggetto della mappa con proprietà differenti, in base al tipo di formato geospaziale passato come argomento.
\\Il lavoro si è poi incentrato sulla classe TypeScript che rappresentava la logica della pagina precedentemente creata, andando ad inserire al suo interno alcune mappe fittizie, tra cui una in formato GeoJSON, così da poterne verificare il suo funzionamento. Tali mappe, che venivano rappresentate come "layer", sono state tutte sovrapposte a quello di default, ovvero quello che conteneva la mappa di OpenStreetMap. In questo modo è stato possibile capire se le altre mappe sovrapposte venivano rappresentate correttamente o meno e riuscire a navigare all'interno delle mappe capendo dove ci si trovava.
