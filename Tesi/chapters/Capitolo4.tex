...
Il candidato, sotto supervisione del tutor aziendale, ha iniziato lo sviluppo del progetto BMS creando all'interno dell'applicazione una nuova pagina web, nella quale ha importato il componente della mappa già esistente, che inizialmente si trovava soltanto nella pagina dell'Ui-Kit, così da poter integrare il supporto ai protocolli OGC richiesti e iniziare a testare le mappe censite.
...
\\Per fare ciò, è stata selezionata come prima mappa da testare quella delle frane in Toscana, fornita da IdroGEO, in quanto tutte quelle fornite da questo ente, possono essere scaricate localmente e caricate all'interno del progetto, senza dover contattare alcun servizio esterno. Ciò facilità il lavoro di programmazione, riducendo il rischio di errore dovuto da eventuali problemi di disservizio dei server di mappe esterni, ad esempio, una mappa fornita dal GeoPortale Nazionale tramite WMS potrebbe risultare momentaneamente non disponibile.
\\In questa fase, le parti di codice che hanno subito più modifiche sono le classi BuildLayer.ts e MapModel.ts, le quali si occupano di far renderizzare ad OpenLayers i vari tipi di mappe con proprietà differenti. Inizialmente, gli unici tipi implementati erano erano Tiles e Assets, i quali venivano
...
\\Il lavoro è continuato con l'implementazione della pagina web descritta precedentemente, andando a stilare al suo interno una sezione con cui poter selezionare le varie mappe disponibili. Come prima mappa, il tirocinante ha inserito all'interno di tale sezione la mappa in formato GeoJSON descritta precedentemente, così da poter verificare se il codice da lui scritto all'interno del componente della mappa era funzionante o meno. Per comprende al meglio tale risultato, la mappa, rappresentata come "layer", è stata sovrapposta a quella di default, ovvero quella fornita da OpenStreetMap. In questo modo è stato possibile comprendere se il layer veniva rappresentato correttamente, riuscendo così a orientarsi all'interno delle mappe stesse in termini geografici.
\\La prima problematica riscontrata in questa operazione è stata quella dell'errata proiezione utilizzata per la rappresentazione della mappa. La mappa in questione utilizzava una proiezione differente rispetto a quella di default posta da OpenLayers, andando così a rappresentare la mappa della Toscana a delle coordinante totalmente differenti rispetto a quelle aspettate. Precisamente, la mappa in formato GeoJSON conteneva al suo interno un insieme di punti distribuite a delle coordinate scritte nella proiezione ESPG:blah. Quest'utlime, venivano lette e interpretate da OpenLayer con una proiezione ESPG:blah, andando così a collocare l'insieme dei punti in un area totalmente errata.
\\Per risolvere questo problema, è stato necessario l'uso di una libreria esterna, nota con il nome di Proj4 (Proj4j per JavaScript) che si occupa di effettuare i calcoli delle proiezioni
