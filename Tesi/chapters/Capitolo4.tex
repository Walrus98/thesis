...
\\Il lavoro è continuato con l'implementazione della pagina web descritta precedentemente, andando a stilare al suo interno una sezione con cui poter selezionare le varie mappe disponibili. Come prima mappa, il tirocinante ha inserito all'interno di tale sezione la mappa in formato GeoJSON descritta precedentemente, così da poter verificare se il codice da lui scritto era funzionante o meno. Per comprende al meglio tale risultato, la mappa, rappresentata come "layer", è stata sovrapposta a quella di default, ovvero quella fornita da OpenStreetMap. In questo modo è stato possibile comprendere se il layer veniva rappresentato correttamente, riuscendo così a orientarsi all'interno delle mappe stesse in termini geografici.
\\La prima problematica riscontrata in questa operazione è stata quella dell'errata proiezione utilizzata per la rappresentazione della mappa. La mappa in questione utilizzava una proiezione differente rispetto a quella di default scelta da OpenLayers, andando così a rappresentare la mappa della toscana a delle coordinante completamente differenti rispetto a quelle aspettate. La mappa quindi possedeva delle coordinate corrette, ma venivano interpretate da OpenLayer con la proiezione sbagliata, andando a collocare la mappa in un area totalmente errata.
