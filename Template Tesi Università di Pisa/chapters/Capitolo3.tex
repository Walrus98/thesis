...
% Spiegare che i servizi forniscono un pososno fonire un insieme di mappe, non una sola.
...
\end{itemize}
Qui di seguito è riportato uno snippet contenente alcuni esempi possibili di richieste
WFS, eseguite utilizzando il comando CURL per simulare il comportamento di una richiesta HTTP.
...
OpenLayers supporta una vasta gamma di sistemi di coordinate geografiche e proiezioni cartografiche, consentendo agli sviluppatori di creare mappe per diverse regioni del mondo e utilizzare diverse proiezioni cartografiche per rappresentare i dati in modo accurato.
...
\\Inoltre, come si può vedere dalla \href{https://openlayers.org/en/latest/apidoc/}{documentazione}, l'architetutra di OpenLayers è basata su un modello di programmazione modulare, in cui ogni ogni modulo svolge precise funzionalità per la creazione e la gestione delle mappe. I moduli di OpenLayer principali sono:
...
OpenLayers supporta anche l'aggiunta di plugin di terze per estendere le sue funzionalità. Una raccolta di plugin è fornita sul loro sito al seguente \href{https://openlayers.org/3rd-party/}{link}.
