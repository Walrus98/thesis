...
\\Ciò può sembrare una cosa negativa, ma è importante tenere conto anche del peso delle richieste. Una richiesta di getCapabilities ad un servizio di mappe può restituire un messaggio di risposta molto grande e pesante, durante il tirocinio è capitato per esempio di ricevere risposte di un peso oltre i 30MB. Anche se può sembrare insignificante, gestire un peso simile all'interno di servizi web risulta molto tedioso, specialmente dal punto di vista della memorizzazione nella cache del browser.
\\Tuttavia, come indicato nella documentazione che spiega la specifica del servizio TMS link, esistono delle richieste più semplici e compatte che un client può per ottenere delle informazioni. Le più importanti sono:
\begin{itemize}
      \item TileMapServiceResource, che fornisce in XML la lista di tutte le mappe disponibili che il servizio tms possiede.
            Per ognuna di questa è definita la proiezione e i l'url diretto da contattare per ricevere le tile
\end{itemize}
Quindi il servizio TMS, in un modo più semplice e coinciso, offre comunque la possibilità ai client di scoprire tutte le possibili mappe e proiezioni associate.
...
Possiamo vedere quindi le tile fornite dal tms come una piramide di immaini multiple a vari livelli di zoom
...
% no get capabilities
% più leggero perché le richieste xml get capabilities sono pesanti e grosse

% questo comporta ad avere meno informazioni lato client
% non conosco le mappe disponibili, non conosco le proiezioni, non
% conosco tutte le richieste che posso fare al servizio wms/wmts/etc
% qui direttamente attraverso l'url chiedo le coordinate della mappa x,y,z

% essendo tutto su unica richiesta, è possibile però esplorare il servizio attraverso l'url proprio da come è fatta la sua struttura
% possiamo vedere le tiles come una piramide di imagini a livelli multipli di zoom
