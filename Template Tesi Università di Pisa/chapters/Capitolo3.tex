...
\end{itemize}
Ciascuno di questi standard offre funzionalità specifiche per soddisfare varie esigenze di accesso ai dati geospaziali.
...
A differenza del WMS, il quale invia un'unica immagine per ogni mappa ai client, il WMTS la "segmenta" in piccole porzioni rettangolari, denominate "Tiles".
\\Quest'ultime vengono prima pre-renderizzate e poi memorizzate sul server. Successivamente, sotto richiesta dei client, vengono recuperate e restituite come messaggio di risposta.
Nello specifico, quando un utente desidera visualizzare una mappa (nel nostro caso in un applicazione web), il client invia tante richieste HTTP al server WMTS, una per ogni tile, specificando la porzione di mappa che vuole visualizzare, la proiezione e il formato dell'immagine desiderato. Il server quindi risponde inviando le tiles corrispondenti alla richiesta del client. A questo punto il client, una volta ricevute le tiles, le combina insieme per formare la porzione di immagine effettiva di mappa richiesta.
% L'utilizzo delle tiles riduce la quantità di elaborazione di dati che il server deve eseguire per servire correttamente la mappa. Questo perché non c'è sarà più bisogno di far generare l'intera mappa al servizio per poi restituirla, ma sarà necessario solo renderizzare in tiles la porzione di mappa richiesta dal client
% Nel caso del servizio WMTS, infatti, basterà restituire solo la porzione di mappa richiesta in tiles
% % Grazie all'utilizzo delle tiles, inoltre, il server WMTS è più efficiente in quanto in base alla richiesta del client, deve solamente generare una porzione di mappa e restituirla in tile, invece di renderizzare l'intera mappa per poi restituirla.
% % Il server WMTS è più efficiente dal punto di vista prestazionale, in quanto il server WMS deve generare tutte le volte l'immagine dell'intera mappa.
% % Nel caso del WMTS
% % Rispetto al Web Map Service (WMS), il WMTS offre diversi vantaggi in termini di efficienza, prestazioni. Innanzitutto, poiché fornisce tiles pre-generati, riduce la quantità di elaborazione richiesta dal server
% % rispetto a WMS, che deve tutte le volte generare
\\Una delle principali caratteristiche del WMTS è la possibilità di utilizzare una politica di caching per le tiles. Essendo piccole porzioni di immagini, queste possono essere facilmente memorizzate nella cache. Sul lato backend, infatti, il servizio WMTS può cachare le tiles più utilizzate e servirle più rapidamente al client. Lato frontend, invece, il browser è in grado di intercettare le risposte inviate dal server e di memorizzare nella sua cache interna l'immagine della tile. In questo modo, quando il client richiede la stessa tile, quest'ultima viene visualizzata senza dover contattare nuovamente il server. Questo è possibile poichè le tiles sono di piccole dimensioni e quindi il peso effettivo della richiesta è sufficientemente da poter essere memorizzato nella cache del browser.
...
\section{Open Source Geospatial Foundation (OSGeo)}

\section{Tile Map Service (TMS)}
...
\\Questa libreria offre delle API che consentono agli sviluppatori di integrare facilmente diversi tipi di servizi geospaziali nei loro progetti. Tra i servizi supportati vi sono il WMS, WFS, WCS, WMTS, TMS e altri ancora.
